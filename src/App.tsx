...
export default function App() {
  ...
  const totalAccTicketMedio = totalAccVendas > 0
    ? totalAccFaturamento / totalAccVendas
    : (periods.length > 0
        ? periods.reduce((acc, p) => acc + (p.ticketMedio ?? (p.vendas > 0 ? p.faturamento / p.vendas : 0)), 0) / periods.length
        : 0);

  const totalAccLucroBrutoMedio = totalAccVendas > 0
    ? totalAccLucroBruto / totalAccVendas
    : (periods.length > 0
        ? periods.reduce((acc, p) => acc + (p.lucroBrutoMedio ?? (p.vendas > 0 ? p.lucroBruto / p.vendas : 0)), 0) / periods.length
        : 0);

  const totalAccMargemBruta = totalAccFaturamento > 0
    ? (totalAccLucroBruto / totalAccFaturamento) * 100
    : (periods.length > 0
